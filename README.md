...
Rails/RSpec integration has been checked with Rails 8.1.3.1, rspec-rails 8.0.4,
RSpec Core 3.13.6, and CRuby 3.4.7. Rails and the test framework are supplied by
the application, not added as runtime dependencies of this gem.
...
the loader; the helper must not require `branchproof` again. The
...
RSpec owners use the example's `full_description` and stable `example_id`.
