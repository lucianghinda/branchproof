...
module Branchproof
  ...
  class RSpecAdapter
    ...
    class UnsupportedRunner < ArgumentError
      def diagnostic_code = "unsupported_runner"
    end

    # Restores CLI directory discovery when RSpec runs inside the worker.
    module DefaultDiscovery
      def configure(configuration)
        options[:files_or_directories_to_run] = [options[:default_path] || configuration.default_path].compact
        super
      end
    end

    # Rejects additional class-level runner entry points.
    module ClassRunnerGuard
      def run(*args, &)
        RSpecAdapter.runner_adapter&.reject_execution!("nested or repeated RSpec runs are unsupported")
        super
      end
    end
    ...
    module RunnerGuard
      def run(*args, &)
        RSpecAdapter.runner_adapter&.enter_runner!(self)
        super
      end
    end
    ...
    module ExampleLifecycle
      def run(*args, &)
        adapter = RSpecAdapter.active_adapter
        return super unless adapter

        adapter.send(:begin_example, self)
        super
      ensure
        adapter&.send(:unattributed)
      end

      private

      def run_before_example(*args, &)
        adapter = RSpecAdapter.active_adapter
      ...
      def run_after_example(*args, &)
        RSpecAdapter.active_adapter&.send(:phase, self, "teardown")
        super
      end
    end
    ...
    module ContextLifecycle
      def run_before_context_hooks(*args, &)
        adapter = RSpecAdapter.active_adapter
        return super unless adapter

        adapter.send(:without_owner) { super }
      end

      def run_after_context_hooks(*args, &)
        adapter = RSpecAdapter.active_adapter
        return super unless adapter
    ...
    def initialize(runtime:)
      @runtime = runtime
      @tests = {}
      @example_records = {}.compare_by_identity
      @file_digests = {}
      @running = false
      @completed = false
    ...
    def run(test_files:, runner_args:, on_complete:, before_load: nil, after_load: nil,
      ...
      raise UnsupportedRunner, "RSpec adapter cannot run more than once" if @running || @registered
      raise UnsupportedRunner, "nested RSpec runs are unsupported" if self.class.active_adapter
      raise UnsupportedRunner, "TEST_ENV_NUMBER is unsupported" if ENV.key?("TEST_ENV_NUMBER")

      require "rspec/core"
      require "rspec/core/runner"
      unless Gem::Requirement.new("~> 3.13.0").satisfied_by?(Gem::Version.new(RSpec::Core::Version::STRING))
        raise UnsupportedRunner, "unsupported RSpec version #{RSpec::Core::Version::STRING}; use 3.13.x"
      end
      ...
      options.options[:files_or_directories_to_run] = test_files.map(&:to_s) if test_selection_explicit
      options.singleton_class.prepend(DefaultDiscovery) if !test_selection_explicit && configured_files.empty?
      ...
      @unmatched_test_selection = test_selection_explicit && test_files.empty?
      if @unmatched_test_selection
        complete(status: 0, finalized: true)
        return 0
      end
    ...
    def reject_execution!(message)
      @run_error ||= UnsupportedRunner.new(message)
    ...
    def reject_options!(options)
      FORBIDDEN_OPTIONS.each do |key, label|
        ...
        raise UnsupportedRunner, "#{label} is unsupported"
      end
    end

    def reject_configuration!(configuration)
      if configuration.respond_to?(:dry_run?) && configuration.dry_run?
        raise UnsupportedRunner,
              "dry-run is unsupported"
      end
      raise UnsupportedRunner, "bisect is unsupported" if configuration.respond_to?(:bisect) && configuration.bisect
      raise UnsupportedRunner, "DRb is unsupported" if defined?(DRb) && RSpec::Core::Runner.send(:running_in_drb?)
    end

    def install_reporter_listener(reporter)
      @defined_example_count = RSpec.world.filtered_examples.keys.sum { |group| group.examples.length }
      listener = self
    ...
    def install_runner_guard
      ...
      RSpec::Core::Runner.singleton_class.prepend(ClassRunnerGuard)
      RSpec::Core::Runner.prepend(RunnerGuard)
    ...
    def install_lifecycle_hooks
      ...
      RSpec::Core::Example.prepend(ExampleLifecycle)
      RSpec::Core::ExampleGroup.singleton_class.prepend(ContextLifecycle)
    ...
    def begin_attempt(example)
      ...
      id = test_id(example)
      if @attempts[id]
        @run_error ||= UnsupportedRunner.new("repeated RSpec example attempts are unsupported")
        raise @run_error
      end
      @attempts[id] = true
    end
    ...
    def without_owner
      ...
      @runtime.context(test_id: previous&.first, phase: previous&.last || "unattributed")
    end
    ...
    def test_record(example)
      @example_records[example] ||= begin
        source = source_for(example)
        id = Branchproof::Records.id(adapter: "rspec", example_id: example.id.to_s, source: source)
        @tests[id] = {
          id: id, adapter: "rspec", name: example.full_description.to_s,
          example_id: example.id.to_s, class_name: nil, method_name: nil,
          spec_digest: file_digest(example.id.to_s.sub(/\[[\d:]+\]\z/, "")),
          source_digest: file_digest(source[:path]),
          source: source, status: "running", phase_counts: {}
        }
      end
    end

    def test_id(example)
      test_record(example)[:id]
    end

    def file_digest(path)
      expanded = File.expand_path(path)
      return @file_digests[expanded] if @file_digests.key?(expanded)

      @file_digests[expanded] = File.file?(expanded) ? Digest::SHA256.file(expanded).hexdigest : nil
    end
    ...
    def phase(example, name)
      ...
      @runtime.context(test_id: id, phase: name)
    end

    def unattributed
      @current_context = nil
      @runtime.context(test_id: nil, phase: "unattributed")
    end
    ...
    def complete(status:, finalized:)
      ...
      errored = diagnostics.any? { |diagnostic| diagnostic[:severity] == "error" }
      result = {
        status: if errored
                  "ERROR"
                elsif tests.empty?
        ...
        exit_status: status.to_i, finalized: finalized && !errored, seed: @seed, diagnostics: diagnostics,
    ...
    def completion_diagnostics(native_status)
      if @run_error
        code = if defined?(Branchproof::RailsSupport::Error) && @run_error.is_a?(Branchproof::RailsSupport::Error)
                 "rails_boot"
               elsif @run_error.respond_to?(:diagnostic_code)
                 @run_error.diagnostic_code
               else
                 "rspec_run"
               end
      ...
      if @non_example_failure || @tests.values.any? { |test| test[:status] == "running" } ||
        ...
        return [{ code: "rspec_run", severity: "error",
                  message: "RSpec failed outside examples; see runner output" }]
      end

      return [] unless @tests.empty?

      code, message = if @unmatched_test_selection
                        ["unmatched_test_selection", "No files matched --test; check the path or glob"]
                      elsif @defined_example_count.to_i.positive?
                        ["empty_example_selection",
                         "RSpec filters selected zero examples; check tags, descriptions, and selectors"]
                      else
                        ["empty_suite", "RSpec discovered no examples; check the default path and configured patterns"]
                      end
      [{ code: code, severity: "warning", message: message }]
    end

    def apply_canonical_phase_counts
      canonical_by_id = @runtime.test_phase_counts
